...
HttpDownloader::DownloadError runGetWolf(const std::string& startUrl, const std::string& username,
                                         ...
                                         uint32_t timeoutMs, bool downgradeRedirectsToHttp) {
  ...
  for (int hop = 0; hop <= MAX_REDIRECTS; ++hop) {
    ...
    // Bearer takes precedence over Basic -- a caller never has a legitimate
    // reason to pass both, and crosspoint-sync's device routes only ever
    // accept the former (see docs/API.md's auth table).
...
HttpDownloader::DownloadError runGet(const std::string& url, const std::string& username, const std::string& password,
  ...
  // Bearer takes precedence over Basic -- see the wolfSSL branch's comment
  // above (runGetWolf) for why a caller never legitimately sends both.
  if (!bearerToken.empty()) {
  ...
  // 30x responses manually. The GitHub release CDN and book download endpoints
  // both redirect.
...
#if defined(FREEINK_NET_WOLFSSL)
  return runGetWolf(url, username, password, bearerToken, sink, timeoutMs, downgradeRedirectsToHttp);
#else
...
HttpDownloader::DownloadError HttpDownloader::downloadToFile(const std::string& url, const std::string& destPath,
  ...
  const DownloadError result =
      runGetSecure(url, username, password, "", sink, DEFAULT_TIMEOUT_MS, downgradeRedirectsToHttp);
