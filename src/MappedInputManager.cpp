...
void MappedInputManager::update() const {
  ...
  // Same frame boundary clears the injected edges, so an injected CMD:PRESS
  // reads true for exactly one loop() iteration however many times an activity
  // queries it within that iteration. Must stay after the long-press sweep
  // above: that sweep calls isPressed(), which still has to see the injected
  // hold this frame.
