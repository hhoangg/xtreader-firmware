...
// Lines of text under each tile. Exactly one, carrying the name and nothing
// else: the uploader and the attach count used to have a second line, but at
// 219 PPI on a 134px-wide tile they read as noise under a picture, and the two
// lines cost more vertical band than a third row of art would. Anything longer
// than the tile is ellipsised by the caller rather than wrapped -- a wrapped
// name made neighbouring tiles different heights for no gain.
constexpr int NAME_LINES = 1;
...
// hints). `nameLineHeight` comes from the renderer's font metrics. The art
// keeps its 134:223 aspect ratio when it has to shrink, so a tile never
// distorts a wallpaper.
Layout layout(const Bounds& content, int nameLineHeight);
